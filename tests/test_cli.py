...
class TestCLIRealWorldSubprocess:
    ...
    def test_cli_as_subprocess_pipe_stdin_stdout(self):
        """Test toggl-to-zoho with pipes (cat input.csv | toggl-to-zoho > output.csv)."""
        import subprocess
        ...
        result = subprocess.run(
            [sys.executable, "-m", "toggl_to_zoho", "-", "-"], input=input_data, capture_output=True
        )
    ...
    def test_cli_as_subprocess_help(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "toggl_to_zoho", "--help"], capture_output=True, text=True
        )
